                ...
                winnerSchema: [
                    {
                        ...
                        options: async () => {
                            return await tenderApi.getTenderLotUsers(this.tender.id, this.lot.id).then(res => {
                                console.log(res);
                                return res.map((bet) => {
                                    return { label: bet.organization.name, price: bet.price, currency_detail: this.tender.currency_detail, value: { bet: bet.id, organization_winner: bet.organization.id } }
                                });
                            }).catch(err => {
                                console.error(err);
                                return [];
